...
const EditSubAdmin: React.FC = () => {
  ...
  const [permissions, setPermissions] = useState<PermissionSet>({
    ...defaultPermissions,
    overview: { view: true },
    finance: { view: true, revenue: true },
  });

  useEffect(() => {
    // Fetch user and permissions here
    console.log('Fetching sub-admin with id:', id);
  }, [id]);

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    console.log('Updating Sub-Admin:', { ...formData, permissions });
    ...
    <div className="space-y-6 max-w-5xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400">Modify permissions and details for {formData.name}.</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 mb-6">Modify the modules and actions this sub-admin can access.</p>
