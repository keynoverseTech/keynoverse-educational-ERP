...
const SupervisorManagement: React.FC = () => {
  ...
  const handleAssignStudents = () => {
    ...
    setSupervisors(supervisors.map(s => 
      s.id === selectedSupervisor.id 
        ? { ...s, assigned: s.assigned + selectedStudents.length }
        : s
    ));
  ...
  // Mock Lecturers (In a real app, this would be filtered by department from API)
  const lecturers = [
  ...
  const availableLecturers = formData.department 
    ? lecturers.filter(l => l.dept === formData.department)
    : [];
  ...
  const filteredSupervisors = supervisors.filter(supervisor => {
    ...
    const matchesFaculty = filterFaculty === 'All' || filterFaculty === 'Faculty of Sciences'; // Mock logic
    const matchesDepartment = filterDepartment === 'All' || supervisor.department === filterDepartment;
    const matchesProgramme = filterProgramme === 'All' || true; // Mock logic
  ...
  const handleAddSupervisor = (e: React.FormEvent) => {
    ...
    if (lecturer) {
      const newSupervisor = {
        ...
        assigned: 0, // Start with 0 assigned students
            ...
            <form onSubmit={handleAddSupervisor} className="p-6 space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setFormData({...formData, department: e.target.value, lecturer: ''})} // Reset lecturer on dept change
              ...
              {/* Lecturer Selection (Filtered by Department) */}
              <div>
                ...
                {formData.department && availableLecturers.length === 0 && (
                   <p className="text-xs text-orange-500 mt-1">No lecturers found in this department.</p>
                )}
              </div>

              {/* Specialization (Optional) */}
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Specialization <span className="text-gray-400 font-normal">(Optional)</span></label>
                <input 
                  type="text"
