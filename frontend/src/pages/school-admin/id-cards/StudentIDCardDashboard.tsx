...
const StudentIDCardDashboard: React.FC = () => {
  ...
  const requests: IDCardRequest[] = [
    { id: 'ENG/2023/042', name: 'Michael Brown', programme: 'Computer Engineering', level: '200L', status: 'Issued', issueDate: '2023-09-15', expiryDate: '2027-08-31', avatar: 'MB' },
    { id: 'MED/2024/005', name: 'Sarah Connor', programme: 'Medicine & Surgery', level: '100L', status: 'Pending', avatar: 'SC' },
    { id: 'LAW/2022/112', name: 'James Smith', programme: 'Law', level: '300L', status: 'Expired', issueDate: '2022-02-10', expiryDate: '2023-02-10', avatar: 'JS' },
  ];
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white flex items-center gap-3">
            ...
            Student ID Cards Management
          </h1>
          <p className="text-gray-500 dark:text-gray-400 mt-1">Manage issuance, reprints, and templates for student identification.</p>
        </div>
        <div className="flex gap-3">
          ...
          <button 
            ...
            className="px-6 py-2 bg-purple-600 text-white rounded-xl font-bold shadow-lg hover:bg-purple-700 transition-colors flex items-center gap-2"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard title="Total Issued" value="2,450" icon={CheckCircle} color="text-green-600" bg="bg-green-100 dark:bg-green-900/20" />
        <StatCard title="Pending Approval" value="142" icon={Clock} color="text-orange-600" bg="bg-orange-100 dark:bg-orange-900/20" />
        <StatCard title="Reprint Requests" value="15" icon={RefreshCcw} color="text-blue-600" bg="bg-blue-100 dark:bg-blue-900/20" />
        <StatCard title="Expired / Lost" value="8" icon={AlertTriangle} color="text-red-600" bg="bg-red-100 dark:bg-red-900/20" />
      </div>
            ...
            {['overview', 'pending', 'issued'].map((tab) => (
              <button
                ...
                className={`px-4 py-2 rounded-lg text-sm font-bold capitalize transition-all ${
                  activeTab === tab
                    ? 'bg-white dark:bg-[#151e32] text-purple-600 shadow-sm'
          ...
          <div className="flex gap-2 w-full md:w-auto">
            <div className="relative flex-1 md:w-64">
              ...
              <input 
                ...
                className="w-full pl-10 pr-4 py-2 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl outline-none focus:ring-2 focus:ring-purple-500/20"
              />
            </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50 dark:bg-gray-900/50 border-b border-gray-100 dark:border-gray-800">
                ...
                <th className="p-4 text-xs font-bold text-gray-500 uppercase tracking-wider">Programme / Level</th>
              ...
              {requests.map((req) => (
                <tr key={req.id} className="hover:bg-gray-50 dark:hover:bg-gray-900/30 transition-colors group">
                  ...
                  <td className="p-4 text-sm text-gray-600 dark:text-gray-300 font-medium">
                    <p>{req.programme}</p>
                    <span className="text-xs text-gray-400">{req.level}</span>
                  </td>
                  <td className="p-4">
                  ...
                  <td className="p-4 text-right">
                    <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      {req.status === 'Issued' && (
                        <button className="p-2 text-gray-400 hover:text-purple-600 hover:bg-purple-50 rounded-lg" title="Preview/Download">
                          <Download size={18} />
                        </button>
                      )}
                      {req.status === 'Pending' && (
