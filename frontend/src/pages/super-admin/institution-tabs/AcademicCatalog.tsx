...
interface AcademicCatalogProps {
  institutionId?: string;
}

const AcademicCatalog: React.FC<AcademicCatalogProps> = ({ institutionId }) => {
  ...
  useEffect(() => {
    ...
    const fetchPrograms = async () => {
      setLoading(true);
      try {
        const response = await superAdminService.getInstitutionPrograms(institutionId);
        const data = Array.isArray(response) ? response : (response as any).data || [];
