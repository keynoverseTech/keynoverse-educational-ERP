...
const SubmitTicket: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    navigate('/student/helpdesk/tickets');
  };
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          <h1 className="text-xl font-bold text-gray-900 dark:text-white">Submit a Ticket</h1>
          <p className="text-sm text-gray-500 dark:text-gray-400">Describe your issue for support</p>
        </div>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-start">
            <div className="md:max-w-xs">
              ...
              <div className="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg p-3 text-center hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors cursor-pointer relative">
                <input 
                  type="file" 
                ...
                <p className="text-[10px] text-gray-400 mt-1">PDF, PNG, JPG up to 5MB</p>
              </div>
            </div>
            ...
            <div className="p-3 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg border border-yellow-100 dark:border-yellow-900/30">
              <div className="flex items-center justify-between mb-1">
                ...
                <div 
                  className={`w-8 h-4 rounded-full relative cursor-pointer transition-colors ${formData.isAnonymous ? 'bg-blue-600' : 'bg-gray-300'}`}
                  onClick={() => setFormData({ ...formData, isAnonymous: !formData.isAnonymous })}
                >
                  <div className={`w-3 h-3 bg-white rounded-full absolute top-0.5 transition-all ${formData.isAnonymous ? 'left-5' : 'left-1'}`}></div>
                </div>
              </div>
              <div className="flex items-start gap-2">
                <AlertCircle className="text-yellow-600 dark:text-yellow-400 shrink-0 mt-0.5" size={16} />
                <p className="text-xs text-gray-600 dark:text-gray-300">
                  If enabled, your identity will be hidden from support staff. We will not be able to follow up directly.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="pt-3 flex justify-end gap-3 border-t border-gray-100 dark:border-gray-700">
            ...
            <button
              type="submit"
              className="flex items-center gap-2 px-5 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors text-sm font-medium shadow-lg shadow-blue-500/30"
            >
