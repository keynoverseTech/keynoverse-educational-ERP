...
export const getErrorDetails = (exception: any): any => {
  if (
    exception.response instanceof ErrorData ||
    exception.message instanceof ErrorData
  ) {
    return (
      (exception.response as ErrorData).details ||
      (exception.message as ErrorData).details
    );
  }
